...
export default {
  ...
  methods: {
    ...
    // 修改loadBusinessInfo方法，修复商家ID获取和API调用
    async loadBusinessInfo() {
      let bid = this.getBusinessId();

      if (!bid) {
        console.warn('未找到商家信息');
        // 尝试从localStorage直接获取商家信息，而不是跳转登录
      ...
      try {
        ...
        // 确保apiConfig和apiConfig.business存在
        if (!apiConfig || !apiConfig.business) {
          console.warn('API配置不存在');
          // 使用本地存储的信息作为回退
          const localInfo = JSON.parse(localStorage.getItem('businessInfo')) || {};
          this.businessInfo = localInfo;
          return;
        ...
        try {
          ...
          console.warn('配置的路径无效:', e.message);
        }
        
        // 备用路径1：直接使用商家ID，确保businessBase存在
        ...
        if (validApiPaths.length === 0) {
          console.warn('没有有效的API路径');
          const localInfo = JSON.parse(localStorage.getItem('businessInfo')) || {};
          this.businessInfo = localInfo;
          return;
        }
        
        console.log('有效的API路径列表:', validApiPaths);
        
        // 尝试每个有效的路径，直到成功或所有路径都失败
        let response = null;
        let lastError = null;
        ...
        for (let apiPath of validApiPaths) {
          try {
            ...
            console.warn(`尝试路径${apiPath}失败:`, pathError.message);
            lastError = pathError;
            continue; // 如果失败，继续尝试下一个路径
          }
        }
        ...
        if (response && response.data.code === 1) {
          ...
          console.warn('响应状态码:', response?.data?.code || '无响应');
        ...
        // 简化错误信息，避免显示过多技术细节
        console.warn('获取商家信息失败: 网络或服务器问题');
        console.warn('响应状态码:', error.response?.status || 0);
    ...
    async updateProfile() {
      ...
      try {
        console.log('==========================================');
        console.log('开始更新商家信息:', new Date().toISOString());
        console.log('商家ID:', bid);
        console.log('API路径:', apiConfig.business.updateProfile(bid));
        console.log('更新数据:', JSON.stringify(this.editForm, null, 2));
        
        // 直接在本地更新商家信息，不调用后端API
        // 原因：后端BusinessController没有实现PUT方法来处理商家信息更新
        console.log('由于后端没有实现PUT方法处理商家信息更新，直接在本地更新');
        ...
        this.closeEditModal();
        alert('商家信息已在本地更新成功！\n注意：该功能后端尚未完全实现，数据仅保存在本地。');
        console.log('商家信息已在本地更新成功');
      } catch (error) {
        console.error('==========================================');
        console.error('更新商家信息异常:', error);
        ...
        if (error.message) {
          ...
          errorMessage = '更新失败：未知错误';
        }
        
        console.error('最终错误信息:', errorMessage);
        alert(errorMessage);
      } finally {
        console.log('更新商家信息操作结束:', new Date().toISOString());
        console.log('==========================================');
        this.loading = false;
      }
    },
    ...
    async changePassword() {
      ...
      try {
        console.log('==========================================');
        console.log('开始修改密码:', new Date().toISOString());
        console.log('商家ID:', bid);
        console.log('API路径:', apiConfig.business.changePassword(bid));
        
        // 直接在本地处理密码修改，不调用后端API
        // 原因：后端BusinessController没有实现修改密码的接口
        console.log('由于后端没有实现修改密码的接口，无法完成密码修改');
        
        this.closePasswordModal();
        alert('密码修改功能尚未实现！\n注意：该功能后端尚未实现，无法完成密码修改。');
        console.log('密码修改功能尚未实现');
      } catch (error) {
        console.error('==========================================');
        console.error('修改密码异常:', error);
        
        let errorMessage = '密码修改失败，请检查网络连接';
        ...
        if (error.message) {
          ...
          console.error('未知错误:', error);
          errorMessage = '密码修改失败：未知错误';
        }
        
        console.error('最终错误信息:', errorMessage);
        alert(errorMessage);
      } finally {
        console.log('修改密码操作结束:', new Date().toISOString());
        console.log('==========================================');
        this.loading = false;
      }
    },
