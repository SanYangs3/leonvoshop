  ...
  <div class="forgot-password-view">
    <div class="fp-container">
      ...
      <div class="fp-box">
        <div class="fp-header">
          <h1>找回密码</h1>
          <p class="subtitle">通过手机号验证码重置您的密码</p>
        </div>
        ...
        <div class="fp-form">
          ...
          >
            ...
            <span class="button-text">{{ isSubmitting ? '重置中...' : '重置密码' }}</span>
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    validateNewPassword() {
      ...
      } else if (this.form.newPassword.length < 6 || this.form.newPassword.length > 16) {
        this.errors.newPassword = '新密码长度为6-16位';
      } else {
        this.errors.newPassword = '';
      }
    },
    ...
    async handleResetPassword() {
      ...
      try {
        const resetRes = await axios.post(
          apiConfig.passwordReset.smsReset,
          {
            phone: this.form.tel,
        ...
        if (resetRes.data && resetRes.data.code === 1) {
          this.showResult = true;
        } else {
          const msg = resetRes.data?.msg || '重置失败，请稍后重试';
          alert('重置失败: ' + msg);
        }
      } catch (error) {
        console.error('找回密码失败:', error);
        alert('重置失败，请检查网络或稍后重试');
      } finally {
