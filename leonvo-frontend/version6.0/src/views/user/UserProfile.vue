  ...
  methods: {
    ...
    async sendSecurityCode() {
      ...
      if (this.passwordSecurity.method === 'phone') {
        ...
        try {
          ...
          const data = res.data || {};
          if (data.code === 1) {
            this.securityMessage = data.msg || '验证码发送成功';
            this.startSecurityCountdown('phone');
          } else {
            this.securityError = data.msg || '验证码发送失败';
          }
        } catch (error) {
          console.error('发送短信验证码失败:', error);
          this.securityError = error.response?.data?.msg || '验证码发送失败，请稍后重试';
        } finally {
    ...
    async submitPasswordReset() {
      ...
      if (!code || code.length !== 6) {
        this.securityError = '请输入6位验证码';
        return;
      }

      if (!newPwd || newPwd.length < 6 || newPwd.length > 16) {
        this.securityError = '新密码长度为6-16位';
        return;
      }

      if (newPwd !== confirmPwd) {
        this.securityError = '两次输入的密码不一致';
        return;
      }
      ...
      try {
        ...
        const data = res.data || {};
        if (data.code === 1) {
          this.securityMessage = data.msg || '密码修改成功，请使用新密码重新登录';
          this.passwordSecurity.code = '';
          this.passwordSecurity.newPassword = '';
          this.passwordSecurity.confirmPassword = '';
        } else {
          this.securityError = data.msg || '密码修改失败';
        }
      } catch (error) {
        console.error('密码修改失败:', error);
        this.securityError = error.response?.data?.msg || '密码修改失败，请稍后重试';
      }
    }
  }
};
