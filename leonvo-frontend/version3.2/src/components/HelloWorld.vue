...
export default {
  ...
  methods: {
    ...
    getImageUrl(picture) {
      ...
      if (picture.includes('unsplash.com')) {
          // 替换失效的unsplash图片为可靠的占位图
    ...
    async fetchAllProducts() {
      ...
      try {
        ...
        this.products = limitedProducts.map(product => ({
          ...
          size: product.size,
          type: product.type,
          heat: product.heat || Math.floor(Math.random() * 100) // 模拟热度，如果没有后端数据
        }));
    ...
    updateCarouselProducts() {
      ...
      console.log('轮播图商品已更新:', this.carouselProducts.map(p => `${p.name} (${p.heat})`));
