          ...
          <div v-else-if="filteredOrders.length > 0" class="orders-list">
            <div v-for="order in filteredOrders" :key="order.oid" class="order-item">
              ...
              <div class="order-actions">
                ...
                <button
                  v-if="order.status === 2 && !order.isReviewed"
                ...
                <div class="rating-input">
                   ...
                   <div class="stars">
                     <span 
                       ...
                       :key="n" 
                       class="star" 
                       :class="{ active: (reviews[item.pid]?.rating || 0) >= n }"
...
export default {
  ...
  methods: {
    ...
    openReviewModal(order) {
      ...
      if (order.items) {
        order.items.forEach(item => {
          this.reviews[item.pid] = { rating: 5, comment: '' };
        });
      }
    ...
    async submitReviews() {
      ...
      try {
        // 模拟提交评价
        // 实际上应该调用后端API: POST /api/comments
        // 这里我们同时更新本地存储的商品热度（评分）

        // 遍历所有评价
        for (const item of this.selectedOrder.items) {
           const review = this.reviews[item.pid];
           if (!review) continue;

           // 计算分数变化
           // 5星+3, 4星+2, 3星+1, 2星+0, 1星-1
           let scoreChange = 0;
           if (review.rating === 5) scoreChange = 3;
           else if (review.rating === 4) scoreChange = 2;
           else if (review.rating === 3) scoreChange = 1;
           else if (review.rating === 2) scoreChange = 0;
           else if (review.rating === 1) scoreChange = -1;

           console.log(`商品 ${item.name} 评分 ${review.rating}, 热度变化 ${scoreChange}`);

           // 更新本地热度 (模拟)
           // 注意：这里我们假设可以通过某种方式更新全局商品数据，
           // 但由于是模拟，我们只能更新localStorage中的缓存，或者调用一个(假)API
           
           // 尝试调用更新商品热度接口(如果存在)
           try {
             // 暂时没有直接更新热度的API，我们这里只做前端提示
             // 真实场景下，提交评论的后端接口会自动处理热度更新
        ...
        // 标记订单已评价 (本地标记，因为后端可能没字段)
        this.selectedOrder.isReviewed = true; // 仅当前会话有效
