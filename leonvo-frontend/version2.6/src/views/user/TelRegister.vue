...
export default {
  ...
  methods: {
    async validateUsername() {
      ...
      // 检查用户名是否已存在（调用后端API）
      try {
        const response = await fetch(`http://localhost:8080/api/user/check-username/${this.form.username}`);
    ...
    async checkTelExists() {
      ...
      try {
        const response = await fetch(`http://localhost:8080/api/user/check-tel/${this.form.tel}`);
        const data = await response.json();
    ...
    async handleRegister() {
      ...
      try {
        const response = await fetch('http://localhost:8080/api/user/register-with-sms', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
...
.button-disabled {
  background: linear-gradient(135deg, rgba(187, 222, 251, 0.8) 0%, rgba(144, 202, 249, 0.8) 100%);
  color: rgba(75, 108, 183, 0.7);
  cursor: not-allowed;
}
