          ...
          <div class="form-section security-section">
            <div class="security-header">
              ...
              <div class="security-method-switch">
                <button
                    :class="{ active: bindMethod === 'email' }"
                    @click="bindMethod = 'email'; bindError = ''; bindMessage = ''"
                >
                  邮箱换绑
                </button>
                <button
                    :class="{ active: bindMethod === 'phone' }"
                    @click="bindMethod = 'phone'; bindError = ''; bindMessage = ''"
                >
                  手机换绑
                </button>
              </div>
            ...
            <div class="security-body">
              ...
              <!-- 邮箱换绑 -->
              <template v-if="bindMethod === 'email'">
                <div class="form-group">
                  <label>
                    <span class="label-icon">📧</span>
                    新邮箱：
                  ...
                  <div class="input-with-action">
                    <input
                        ...
                        class="form-input"
                        placeholder="请输入要换绑的新邮箱"
                    />
                    <button
                        class="verify-btn"
                        :disabled="bindCountdown.email > 0 || isSendingBindEmail"
                        @click="sendBindCode('email')"
                    >
                      <span v-if="bindCountdown.email > 0">{{ bindCountdown.email }}s</span>
                ...
                <div class="form-group">
                  ...
                  <div class="input-with-action">
                    <input
                        ...
                        class="form-input"
                        placeholder="请输入要换绑的新手机号"
                    />
                    <button
                        class="verify-btn"
                        :disabled="bindCountdown.phone > 0 || isSendingBindPhone"
                        @click="sendBindCode('phone')"
                    >
                      <span v-if="bindCountdown.phone > 0">{{ bindCountdown.phone }}s</span>
                      <span v-else>发送验证码</span>
                    </button>
                  </div>
                </div>
              ...
              <div class="action-buttons">
                <button class="save-btn" @click="confirmBind(bindMethod)">
                  <span class="btn-icon">✅</span>
                  确认换绑
                </button>
              </div>

              <div v-if="bindError" class="error-message">
                {{ bindError }}
              </div>
              <div v-if="bindMessage" class="success-message">
                {{ bindMessage }}
              </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      bindMessage: '',
      bindError: '',
      isSendingBindEmail: false,
      isSendingBindPhone: false,
  ...
  beforeUnmount() {
    if (this.bindTimers.email) clearInterval(this.bindTimers.email);
    if (this.bindTimers.phone) clearInterval(this.bindTimers.phone);
  },
  computed: {
    membershipTiers() {
  ...
  methods: {
    ...
    startBindCountdown(type) {
      if (this.bindTimers[type]) clearInterval(this.bindTimers[type]);
      this.bindCountdown[type] = 60;
      this.bindTimers[type] = setInterval(() => {
        this.bindCountdown[type] -= 1;
        if (this.bindCountdown[type] <= 0) {
          clearInterval(this.bindTimers[type]);
          this.bindTimers[type] = null;
          this.bindCountdown[type] = 0;
        }
      }, 1000);
    },

    async sendBindCode(type) {
      this.bindError = '';
      this.bindMessage = '';

      if (!this.bindForm.currentPassword) {
        this.bindError = '请先输入当前密码';
        return;
      }

      if (type === 'email') {
        const email = this.normalizeToString(this.bindForm.newEmail);
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) {
          this.bindError = '请输入有效的邮箱';
          return;
        }
        this.isSendingBindEmail = true;
        try {
          ...
          const data = res.data || {};
          if (data.code === 1) {
            this.bindMessage = data.msg || '验证码已发送';
            this.startBindCountdown('email');
          } else {
            this.bindError = data.msg || '发送验证码失败';
          }
        } catch (e) {
          console.error('发送邮箱绑定验证码失败:', e);
          this.bindError = e.response?.data?.msg || '发送验证码失败，请稍后重试';
        } finally {
          this.isSendingBindEmail = false;
        }
      } else {
        const phone = this.normalizeToString(this.bindForm.newPhone);
        const phoneRegex = /^1[3-9]\d{9}$/;
        if (!phoneRegex.test(phone)) {
          this.bindError = '请输入有效的手机号';
          return;
        }
        this.isSendingBindPhone = true;
        try {
          ...
          if (data.code === 1) {
            this.bindMessage = data.msg || '验证码已发送';
            this.startBindCountdown('phone');
          } else {
            this.bindError = data.msg || '发送验证码失败';
          }
        } catch (e) {
          console.error('发送手机绑定验证码失败:', e);
          this.bindError = e.response?.data?.msg || '发送验证码失败，请稍后重试';
        } finally {
          this.isSendingBindPhone = false;
        }
      }
    },
    ...
    async confirmBind(type) {
      ...
      try {
        let url = '';
        const payload = { uid, currentPassword: this.bindForm.currentPassword };

        if (type === 'email') {
          const email = this.normalizeToString(this.bindForm.newEmail);
          const code = this.normalizeToString(this.bindForm.emailCode);
          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
          if (!emailRegex.test(email)) {
            this.bindError = '请输入有效的邮箱';
            return;
          ...
          if (!code || code.length !== 6) {
            this.bindError = '请输入6位邮箱验证码';
            return;
          }
          url = `${apiConfig.baseURL}/api/bind/change/email`;
          payload.newEmail = email;
          payload.code = code;
        } else {
          const phone = this.normalizeToString(this.bindForm.newPhone);
          const code = this.normalizeToString(this.bindForm.phoneCode);
          const phoneRegex = /^1[3-9]\d{9}$/;
          if (!phoneRegex.test(phone)) {
            this.bindError = '请输入有效的手机号';
            return;
          ...
          if (!code || code.length !== 6) {
            this.bindError = '请输入6位短信验证码';
            return;
          }
          url = `${apiConfig.baseURL}/api/bind/change/phone`;
        ...
        const data = res.data || {};
        if (data.code === 1) {
          this.bindMessage = data.msg || '换绑成功';
          await this.loadUserInfo();
          this.bindForm.emailCode = '';
          this.bindForm.phoneCode = '';
        } else {
          this.bindError = data.msg || '换绑失败';
        }
      } catch (e) {
        console.error('换绑失败:', e);
        this.bindError = e.response?.data?.msg || '换绑失败，请稍后重试';
      }
    }
  }
};
