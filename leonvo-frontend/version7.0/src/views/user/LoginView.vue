        ...
        <div class="login-form">
          ...
          <div v-if="loginMethod === 'phone'" class="form-group">
            ...
            <div class="code-row">
              ...
              >
                <span v-if="phoneCodeCountdown > 0">{{ phoneCodeCountdown }}s后重发</span>
          ...
          <div v-if="loginMethod === 'email'" class="form-group">
            ...
            <div class="code-row">
              ...
              >
                <span v-if="emailCodeCountdown > 0">{{ emailCodeCountdown }}s后重发</span>
          ...
          <!-- 密码 -->
          <div class="form-group" v-if="loginMethod === 'account'">
          ...
          <div class="test-account">
            <div class="test-header">
              ...
              <div class="test-title">联系客服</div>
            </div>
            <p class="test-subtitle">
              如果有问题请联系我们的经理：沐衡（大概率联系不上XD）
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      phoneInfo: { phone: '', code: '' },
      emailInfo: { email: '', code: '' },
      password: '',
      rememberMe: false,
      showSuccess: false,
      currentUsername: '',
  ...
  computed: {
    isButtonDisabled() {
      if (this.loginMethod === 'account') {
        ...
        return !this.phoneInfo.phone.trim() || (this.phoneInfo.code || '').length !== 6;
      } else {
        return !this.emailInfo.email.trim() || (this.emailInfo.code || '').length !== 6;
      }
    },
    // 获取登录接口URL（账号为GET；电话/邮箱为验证码登录POST）
    loginConfig() {
      if (this.loginMethod === 'account') {
        return {
          url: apiConfig.user.usernameLogin,
          method: 'GET'
        };
      } else if (this.loginMethod === 'phone') {
        return {
          url: apiConfig.sms?.loginCode || `${apiConfig.baseURL}/api/sms/login-code`,
          method: 'POST'
        };
      } else {
        return {
          url: apiConfig.email?.loginCode || `${apiConfig.baseURL}/api/email/login-code`,
          method: 'POST'
        };
      }
    }
  ...
  methods: {
    async handleLogin() {
      ...
      try {
        ...
        let requestConfig = {};
        let loginIdentifier = ''; // 用于后续获取用户信息

        // 根据登录方式设置参数和请求方式
        if (this.loginMethod === 'account') {
          // 用户名登录使用GET，查询参数
          loginIdentifier = this.accountInfo.username.trim();
          requestConfig = {
            method: 'GET',
            url: loginConfig.url,
            params: {
          ...
          // 手机号 + 验证码 登录（POST）
          loginIdentifier = this.phoneInfo.phone.trim();
          requestConfig = {
            method: 'POST',
            url: loginConfig.url,
            data: { phone: loginIdentifier, code: this.phoneInfo.code },
            headers: { 'Content-Type': 'application/json' }
          };
        } else {
          // 邮箱 + 验证码 登录（POST）
          loginIdentifier = this.emailInfo.email.trim();
          requestConfig = {
            method: 'POST',
            url: loginConfig.url,
            data: { email: loginIdentifier, code: this.emailInfo.code },
            headers: { 'Content-Type': 'application/json' }
          };
        }

        console.log('登录请求配置:', requestConfig);
        ...
        if (response.data && response.data.code === 1) {
          // 后端登录接口只返回成功/失败，不返回用户数据
          // 需要根据登录信息去获取用户详情
          await this.handleLoginSuccessAndGetUserInfo(loginIdentifier, requestConfig.params || requestConfig.data || {});
        } else {
          // 登录失败
    ...
    // 登录成功后获取用户信息
    async handleLoginSuccessAndGetUserInfo(loginIdentifier, loginParams) {
      try {
        // 根据登录方式获取用户信息
        let userData = null;
        
        if (this.loginMethod === 'account') {
          // 用户名登录：需要先获取所有用户，然后找到匹配的用户
          const allUsersResponse = await axios.get(apiConfig.user.getAllUsers);
          if (allUsersResponse.data && allUsersResponse.data.code === 1) {
            const allUsers = allUsersResponse.data.data || [];
            userData = allUsers.find(user => user.username === loginIdentifier);
          }
        } else if (this.loginMethod === 'phone') {
          // 手机号验证码登录：按手机号匹配用户
          const allUsersResponse = await axios.get(apiConfig.user.getAllUsers);
          if (allUsersResponse.data && allUsersResponse.data.code === 1) {
            const allUsers = allUsersResponse.data.data || [];
            userData = allUsers.find(user => user.phone === loginIdentifier);
          }
        } else {
          // 邮箱验证码登录：按邮箱匹配用户
          const allUsersResponse = await axios.get(apiConfig.user.getAllUsers);
          if (allUsersResponse.data && allUsersResponse.data.code === 1) {
            const allUsers = allUsersResponse.data.data || [];
            userData = allUsers.find(user => user.email === loginIdentifier);
          }
        }
        ...
        if (userData.role && userData.role !== 'user') {
          this.handleLoginFailure({
            code: 0,
            msg: '管理员账户不能登录普通用户端，请使用管理员登录页面'
          });
          return;
        }

        // 调用成功处理函数
        this.handleLoginSuccess(loginParams, userData);
      } catch (error) {
        console.error('获取用户信息失败:', error);
        this.handleLoginFailure({
          code: 0,
          msg: '登录成功但获取用户信息失败，请刷新页面重试'
        });
      }
    },

    handleLoginSuccess(loginData, userData) {
      // 确保userData存在且包含必要信息
      if (!userData) {
        this.handleLoginFailure({
          code: 0,
      ...
      // 如果后端没有返回用户名，根据登录方式获取
      if (!username && loginData) {
        if (this.loginMethod === 'account' && loginData.username) {
          username = loginData.username;
        } else if (this.loginMethod === 'phone' && loginData.phone) {
          username = loginData.phone;
        } else if (this.loginMethod === 'email' && loginData.email) {
      ...
      // 必须保存用户ID（uid），这是后续操作的关键
      if (!userData.uid && !userData.id) {
        console.error('警告：用户信息中未包含用户ID（uid或id）');
        this.handleLoginFailure({
          code: 0,
          msg: '登录失败：未获取到用户ID，请联系管理员'
        });
        return;
      }

      // 保存用户ID（重要！）后端可能使用uid或id字段
    ...
    handleLoginFailure(responseData) {
      ...
      if (this.loginMethod === 'account') {
        ...
        errorMsg += '请检查验证码是否正确或已过期\n';
      } else {
        errorMsg += '请检查验证码是否正确或已过期\n';
      }

      alert(errorMsg);
      if (this.loginMethod === 'account') this.password = '';
      if (this.loginMethod === 'phone') this.phoneInfo.code = '';
      if (this.loginMethod === 'email') this.emailInfo.code = '';
    },
    ...
    async sendPhoneLoginCode() {
      const phone = (this.phoneInfo.phone || '').trim();
      ...
      try {
        const res = await axios.post(apiConfig.sms.sendCode, {
          phone,
          purpose: 'login'
        }, { headers: { 'Content-Type': 'application/json' } });
        const data = res.data || {};
        if (data.code === 1) {
          this.startCodeCountdown('phone');
        } else {
          alert(data.msg || '发送验证码失败');
        }
      } catch (e) {
        console.error('发送短信验证码失败:', e);
        alert(e.response?.data?.msg || '发送验证码失败，请稍后重试');
      } finally {
    ...
    async sendEmailLoginCode() {
      const email = (this.emailInfo.email || '').trim();
      ...
      try {
        const res = await axios.post(apiConfig.email.sendCode, {
          email,
          purpose: 'login'
        }, { headers: { 'Content-Type': 'application/json' } });
        const data = res.data || {};
        if (data.code === 1) {
          this.startCodeCountdown('email');
        } else {
          alert(data.msg || '发送验证码失败');
        }
      } catch (e) {
...
.verify-btn:disabled {
  background: linear-gradient(135deg, #cfd8e2 0%, #b5c4d5 100%);
  cursor: not-allowed;
  color: #6b7a99;
  box-shadow: none;
}
