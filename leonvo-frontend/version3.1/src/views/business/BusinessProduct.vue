  ...
  methods: {
    ...
    async submitProduct() {
      ...
      try {
        ...
        if (response.data.code === 1) {
          this.$message.success(this.newProduct.pid ? '商品更新成功！' : '商品添加成功！')
          this.closeAddModal()
          this.loadProducts()
        } else {
          this.$message.error(`${this.newProduct.pid ? '更新' : '添加'}失败: ${response.data.message}`)
        }
      } catch (error) {
