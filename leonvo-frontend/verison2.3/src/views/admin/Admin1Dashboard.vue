...
const API_ENDPOINTS = {
  ...
  userDistribution: apiConfig.order.getTodayOrdersByProvince,

  // 新增趋势接口
  salesTrend: apiConfig.statistics.getSalesTrend || apiConfig.order.getSalesTrend, // 优先使用统计接口
  userGrowthTrend: apiConfig.statistics.getUserGrowthTrend,

  // 暂无API（用null表示）
  totalUsers: apiConfig.user.getTotalUsers, // 实际存在
  newUsers: apiConfig.user.getTodayNewUsers, // 实际存在
...
const MOCK_DATA = {
  ...
  userGrowth: [
    { date: '01-01', users: 3200 },
    { date: '01-02', users: 3250 },
    { date: '01-03', users: 3310 },
    { date: '01-04', users: 3380 },
    { date: '01-05', users: 3420 },
    { date: '01-06', users: 3480 },
    { date: '01-07', users: 3542 }
  ],
  userDistribution: [
...
async function fetchData(endpoint, mockData, dataKey = null, defaultValue = null) {
  ...
  try {
    ...
    if (response.data.code === 1) {
      ...
      // 处理特殊格式的数据
      if (dataKey && typeof apiData === 'undefined') {
    ...
    if (response.data.code === 1) {
      ...
      if (Array.isArray(apiData)) {
        ...
        const formattedData = apiData.map(item => {
          return {
            month: item.month || item.date || item.time || '',
...
async function fetchUserComparison() {
  try {
    ...
    if (response.data.code === 1) {
      ...
      // 处理不同的数据格式
      if (Array.isArray(apiData)) {
        // 如果是数组格式，确保每个元素都有date和users属性
        const formattedData = apiData.map(item => {
          // 如果已经有date和users属性，直接返回
          if (item.date && item.users !== undefined) {
            return item;
          }

          // 尝试从其他属性映射
          let date = item.date || item.month || item.time || item.day || '';
          let users = item.users !== undefined ? item.users :
        ...
        if (apiData.data && Array.isArray(apiData.data)) {
          growthData = apiData.data;
        } else if (apiData.todayCount !== undefined && apiData.yesterdayCount !== undefined) {
          // 对比数据格式，创建简单的趋势数据
          growthData = [
            { date: '昨日', users: apiData.yesterdayCount || 0 },
            { date: '今日', users: apiData.todayCount || 0 }
          ];
        } else {
          // 使用模拟数据
          growthData = MOCK_DATA.userGrowth;
        }
...
export default {
  ...
  methods: {
    ...
    calculateGrowthRate() {
      if (this.dashboardData.userGrowth.noApi ||
          !this.dashboardData.userGrowth.data ||
          !Array.isArray(this.dashboardData.userGrowth.data) ||
          this.dashboardData.userGrowth.data.length < 2) {
        return 0;
      }

      // 确保 data 是数组且有 users 属性
      const data = this.dashboardData.userGrowth.data;

      // 查找第一个和最后一个有 users 属性的元素
      ...
      const last = lastItem.users || first;
