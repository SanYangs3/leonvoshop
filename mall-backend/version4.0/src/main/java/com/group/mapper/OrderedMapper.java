...
public interface OrderedMapper {
    ...
    */
    ...
    @Select("SELECT " +
            ...
            "    (SELECT COUNT(*) FROM ordered WHERE DATE(order_time) <= t.date " +
            "        AND YEARWEEK(order_time, 1) = YEARWEEK(CURDATE(), 1)) as cumulative_count " +
            "FROM ordered t " +
    ...
    @Select("SELECT " +
            ...
            "        AND MONTH(order_time) = MONTH(CURDATE())) as cumulative_count " +
            "FROM ordered t " +
