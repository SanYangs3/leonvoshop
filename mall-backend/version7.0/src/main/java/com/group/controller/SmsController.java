...
public class SmsController {
    ...
    /**
     * 短信验证码登录（先用 purpose=login 调 /api/sms/send-code 获取验证码）
     */
    @PostMapping("/login-code")
    public Result<Object> loginBySmsCode(@RequestBody SmsCodeLoginRequest request) {
        ...
        boolean verified = smsService.verifyCode(request.getPhone().trim(), request.getCode());
        if (!verified) {
            return Result.error("验证码错误或已过期");
        }
