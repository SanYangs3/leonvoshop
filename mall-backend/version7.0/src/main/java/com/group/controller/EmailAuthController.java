...
public class EmailAuthController {
    ...
    /**
     * 邮箱验证码登录（先用 purpose=login 调 /api/email/send-code 获取验证码）
     */
    @PostMapping("/login-code")
    public Result<Object> loginByCode(@RequestBody EmailCodeLoginRequest request) {
        if (request.getEmail() == null || request.getEmail().trim().isEmpty()) {
            return Result.error("邮箱不能为空");
        }
        ...
        try {
            boolean verified = emailVerificationService.verify(request.getEmail().trim(), "login", request.getCode());
            if (!verified) {
                return Result.error("验证码错误或已过期");
            }
            if (userService.searchEmail(request.getEmail().trim()) == 0) {
