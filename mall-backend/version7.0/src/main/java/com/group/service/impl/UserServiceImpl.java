...
public class UserServiceImpl implements UserService {
    ...
    public void updatePasswordByEmail(String email, String password) {
        int updated = userMapper.updatePasswordByEmail(email, password, LocalDateTime.now());
        if (updated == 0) {
            throw new BusinessException("邮箱不存在或密码未更新");
        }
    }

    @Override
    public void updatePasswordByPhone(String phone, String password) {
        int updated = userMapper.updatePasswordByPhone(phone, password, LocalDateTime.now());
        if (updated == 0) {
            throw new BusinessException("手机号不存在或密码未更新");
        }
    }

    @Override
    public void updateEmailByUid(Long uid, String email) {
        int updated = userMapper.updateEmailByUid(uid, email, LocalDateTime.now());
        if (updated == 0) {
            throw new BusinessException("用户不存在或邮箱未更新");
        }
    }

    @Override
    public void updatePhoneByUid(Long uid, String phone) {
        int updated = userMapper.updatePhoneByUid(uid, phone, LocalDateTime.now());
        if (updated == 0) {
            throw new BusinessException("用户不存在或手机号未更新");
        }
    }
