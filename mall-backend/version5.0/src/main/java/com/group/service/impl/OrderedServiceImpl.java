...
public class OrderedServiceImpl implements OrderedService {
    ...
    public Integer createOrder(Integer uid, List<OrderItem> items,
        ...
        for (OrderItem item : items) {
            ...
            // 强制以商品真实所属商家为准，避免外部传入错误bid
            Integer productBid = product.getBid();
            if (productBid == null) {
                // 如果商品没有关联商家，默认为自营/系统商家 (ID=1)
                productBid = 1;
            }
            item.setBid(productBid);
        ...
        if (order != null) {
            ...
            try {
                ...
                if (logistics != null) {
                    ...
                    logisticsInfo.put("created_time", logistics.getCreatedTime());
                    logisticsInfo.put("updated_time", logistics.getUpdatedTime());
    ...
    public boolean shipOrder(Integer oid, String logisticsCompany, String trackingNumber, String note) throws Exception {
        ...
        // 3. 更新订单状态为已发货 (1)
        int result = orderedMapper.updateOrderStatus(oid, 1);
        return result > 0;
    }

    @Override
    public OrderLogistics getLogisticsByOrderId(Integer oid) {
        return orderLogisticsMapper.selectByOrderId(oid);
    }
}